...
# Reviewed homonym exceptions are the only cards allowed to carry a non-empty
# registry variant. The value is the exact variant label written into the
# registry row.
REVIEWED_HOMONYM_VARIANTS: Final[dict[tuple[str, str, str], frozenset[str]]] = {
...
def reviewed_homonym_variant(
    ...
    """Return the reviewed variant label for a card, or '' if none applies."""
    ...
    allowed = REVIEWED_HOMONYM_VARIANTS.get((word_key, cefr_key, list_key))
