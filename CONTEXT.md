...
dictionary entry as an indivisible unit. Candidate identity includes source,
source headword, entry, POS, accent, IPA, and audio URL. Selection prefers exact
headword/POS evidence and never borrows IPA from one entry while retaining an
audio filename inherited from another.
_Avoid_: top-level IPA fallback, filename-selected pronunciation, mixed-entry audio

**Pronunciation Selection Lock**:
The fingerprint-bound reviewed decision for one `(Card GUID, accent)` when the
best Entry-Scoped Pronunciation is ambiguous, uses an explicit source-headword
alias, or is genuinely absent. A lock either selects one exact candidate or
records `no_pronunciation`; stale candidate sets fail closed.
_Avoid_: IPA override, fuzzy lemma fallback, filename exception

**Pronunciation Media Fingerprint**:
The content identity of one selected pronunciation payload: dictionary source,
normalized source headword, accent, IPA, and audio URL. It is distinct from an
entry-selection fingerprint, so several dictionary entry identities may share
one attested filename and byte sequence only when their Pronunciation Media
Fingerprint is identical.
_Avoid_: filename hash, entry identity, audio basename

**Headword Audio Manifest**:
The generated byte-attestation ledger for every selected Entry-Scoped
Pronunciation. Each row binds its entry-selection fingerprint and Pronunciation
Media Fingerprint to dictionary/entry identity, source, accent, IPA, audio URL,
media filename, byte count, and SHA-256. Production pronunciation is complete
only when the resolver, selection locks, manifest, and local media all agree
exactly; unselected manifest rows fail closed.
_Avoid_: audio directory inventory, filename authority, download cache
...
**Card Example Floor**:
The card-level requirement that a card with `N` distinct POS values has at
least `N` non-empty main Examples across all Semantic Senses. Multiple examples
inside one sense count separately; Idiom Examples do not count. The rule checks
the whole card and does not require every individual Semantic Sense to match its
own POS count.
...
**OPAL Membership**:
Oxford's written (`W`) and spoken (`S`) academic-list membership for one entry
POS. An Oxford Record stores it as `opal: null` or a POS-keyed object whose
values are `['W']`, `['S']`, or `['W', 'S']`; a multi-POS record must not let a
badge from one entry POS leak onto another. The build unions only the final
card POS memberships and emits the established flat tags `OPAL_W` and
`OPAL_S`.
_Avoid_: word-level OPAL flag, cross-POS OPAL fallback
...
- For reviewed primary/secondary semantic pairs, `primary` stays in the normal
  list deck and does not carry `SecondarySense`; every `secondary_*` variant
  routes to `English Academic Vocabulary::Oxford::Oxford 5000::Secondary
  Senses` and carries `SecondarySense`.
...
- **Reviewed semantic variants — `denial|C1|Oxford_5000`**: GUID
  ``$|_`hdAC|%`` is variant `primary` and keeps the statement/act of saying
  something is untrue. GUID `Jhp@WXA!ga` is variant
  `secondary_entitlement_psychological`, keeps both entitlement-refusal and
  unpleasant-truth senses, routes to `English Academic
  Vocabulary::Oxford::Oxford 5000::Secondary Senses`, and carries
  `SecondarySense`.
- **Reviewed semantic variants — `alien|C1|Oxford_5000|adjective`**: GUID
  `Y&tBh??_,}` is variant `primary` and keeps the unfamiliar plus disapproving
  foreign/cultural senses. GUID `q?0?C/TI0}` is variant
  `secondary_disapproving_space`, keeps the unacceptable plus outer-space
  senses, routes to the Secondary Senses deck, and carries `SecondarySense`.
  The separate `alien|B2|Oxford_5000|noun` identity is unchanged.
- **Reviewed semantic variants — `sensitivity|C1|Oxford_5000`**: GUID
  `fM]>3mcy3=` is variant `primary` and keeps the social/emotional senses,
  including the reviewed semicolon merge of personal and informational
...
A fingerprint-bound reviewed transaction that replaces one active Card Identity
with an adjacent primary/secondary pair while preserving complete Semantic
Source Coverage. It partitions or explicitly merges Semantic Senses, assigns a
new canonical GUID/variant/deck to the secondary card, and publishes Card
Registry plus Bilingual Semantic Audit together with rollback on partial
failure. Downstream registries and build outputs are regenerated afterward.
_Avoid_: hand-added registry row, partial split, copy-and-delete card
...
The shared fail-closed check for lossy Unicode in reviewed or learner-facing
text. It rejects `U+FFFD` and an ASCII `?` embedded immediately before a letter,
while allowing legitimate terminal question marks. Canonical review files stay
UTF-8; visual rendering cannot repair bytes already corrupted upstream.
_Avoid_: font-only diagnosis, replacement-character acceptance, ASCII review pipe
...
**Oxford Record**:
A consolidated word entry in `data/sources/oxford.jsonl` produced by parsing the Oxford cache and merging multi-file homonyms. Carries the full per-def schema (`n`, `sensenum_local`, `is_idiom`, `text`, `examples`, `cefr`) and Oxford-only fields populated (`oxford_lists`, POS-scoped `opal`, `awl`, `register_tags`, `topics`). `source = "oxford"`.
